    ...
    <div className="App">
      ...
      <article>
        <section className="pros">
          {proArgs.map((arg, index) => (
            <Argument type={arg.type} key={`arg-${index}`} text={arg.title} />
          ))}
          <button
            onClick={() => handleClick("pro")}
            className="secondary pro-btn"
          >
            Add pro argument
          </button>
        </section>
        ...
        <section className="cons">
          {conArgs.map((arg, index) => (
            <Argument type={arg.type} key={`arg-${index}`} text={arg.title} />
          ))}
          <button
            onClick={() => handleClick("cons")}
            className="secondary cons-btn"
          >
            Add cons argument
          </button>
        </section>
      ...
      {showModal && (
        <ArgumentModal
          ...
          handleSubmitArg={handleSubmitArg}
        />
      )}
    </div>
  );
